...
order = [
    doc.index(f"## {h}")
    for h in ["Task", "Status", "Findings", "Decisions", "Dead ends", "Open questions"]
]
...
check(
    "a.py:1 — does X" in body and "b.py:2 — does Y" in body,
    "append accumulates findings",
)
...
check(
    sc.read_section(doc, "status").strip() == "step 2: editing", "replace swaps status"
)
check(
    "a.py:1 — does X" in sc.read_section(doc, "findings"),
    "replace leaves findings intact",
)
...
check(
    "wrote it as a combo" in sc.read_section(doc, "status"),
    "combo content lands in status",
)
doc = sc.merge(doc, "bogus", "stray note", "append")
check(
    "stray note" in sc.read_section(doc, "findings"),
    "unknown content lands in findings",
)
